...
exports.EnhancedHRPromptEngine = void 0;
class EnhancedHRPromptEngine {
    /**
     * Advanced system prompt with role-specific intelligence
     */
    getSystemPrompt(userRole = 'hr_manager') {
        const rolePrompts = {
...
Your insights are strategic, forward-thinking, and focused on competitive advantage and long-term organizational success.`
        ...
        return rolePrompts[userRole] || rolePrompts.hr_manager;
    }
    /**
     * Dynamic analysis prompt with contextual intelligence
     */
    createAdvancedAnalysisPrompt(context, analysisType, userRole = 'hr_manager') {
...
🏆 **Certifications:** ${person.certifications?.map((c) => c.name).join(', ') || 'None listed'}
...
${similarPersons.slice(0, 5).map((similar, index) => `${index + 1}. ${similar.personName} | Similarity: ${similar.similarity}% | Role: ${similar.role || 'Not specified'}`).join('\n')}
...
**Begin your comprehensive analysis now:**`;
     ...
     */
    createAdvancedReportPrompt(context, reportType, userRole = 'hr_manager') {
        // Handle both old format (with person property) and new format (direct properties)
        const person = context.person || context;
...
Generate a professional, executive-ready report now:`;
     ...
     */
    getAnalysisSpecificInstructions(analysisType, userRole) {
        const instructions = {
            capability_analysis: `
...
- Create adaptive improvement strategies`
        ...
        return instructions[analysisType] || instructions.capability_analysis;
    }
    /**
...
5. 📈 **STRATEGIC POSITIONING** (Market differentiation strategies)`
    ...
    getEducationSummary(education) {
        ...
        const highest = education.reduce((prev, curr) => this.getEducationLevel(curr.degree) > this.getEducationLevel(prev.degree) ? curr : prev);
    ...
    prioritizeTechnologies(technologies) {
    ...
    calculateMarketPosition(person, similarPersons) {
        const avgSimilarity = similarPersons.reduce((sum, p) => sum + p.similarity, 0) / similarPersons.length;
        if (avgSimilarity > 80)
            return 'Top Tier (Highly competitive profile)';
        if (avgSimilarity > 60)
            return 'Above Average (Strong market position)';
        if (avgSimilarity > 40)
            return 'Average (Standard market positioning)';
        return 'Unique Profile (Distinctive market positioning)';
    }
    buildContextualData(person, similarPersons, skillsContext, marketData) {
...
Key Skills: ${person.skills?.slice(0, 5).map((s) => s.skillName).join(', ') || 'None'}
Technologies: ${person.technologies?.slice(0, 5).map((t) => t.technologyName).join(', ') || 'None'}
`;
        if (similarPersons?.length > 0) {
            context += `
🔍 **PEER COMPARISON DATA:**
${similarPersons.slice(0, 3).map((p, i) => `${i + 1}. ${p.personName} (${p.similarity}% similarity)`).join('\n')}
`;
        }
        if (skillsContext) {
            context += `
...
exports.EnhancedHRPromptEngine = EnhancedHRPromptEngine;
