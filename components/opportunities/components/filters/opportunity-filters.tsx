						...
						<div className='flex items-center gap-2'>
							...
							<Input
								...
								onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
									updateClientInput(e.target.value)
								}
								className='w-48'
							/>
						</div>
					...
					<div className='mt-4 pt-4 border-t text-sm text-gray-600'>
						...
						{[
							...
							filters.needsHire !== "all" &&
								`Needs Hire: ${filters.needsHire === "yes" ? "Yes" : "No"}`,
							filters.probability &&
								(filters.probability[0] !== 0 ||
									filters.probability[1] !== 100) &&
								`Probability: ${filters.probability[0]}-${filters.probability[1]}%`,
						]
							.filter(Boolean)
							.join("  ·  ")}
						{!isFiltersValid && (
