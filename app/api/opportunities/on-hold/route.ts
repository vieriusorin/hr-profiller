...
export async function GET(request: Request) {
  try {
    ...
    const response = await fetch(`${JSON_SERVER_URL}/opportunities?status=On Hold`);
    if (!response.ok) {
      throw new Error(`Failed to fetch from json-server: ${response.statusText}`);
    }
    const allOnHoldOpportunities: Opportunity[] = await response.json();

    const filteredOpportunities = applyFilters(allOnHoldOpportunities, filters).map(opp => ({
      ...opp,
    ...
    console.error('Failed to fetch on-hold opportunities:', errorMessage);
    return NextResponse.json({ error: `Failed to fetch on-hold opportunities: ${errorMessage}` }, { status: 500 });
  }
} 
